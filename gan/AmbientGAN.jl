...
block_pixel(x, p) = map(x -> rand() < p ? 0.0 : x, x)
measure(x) = block_pixel(x, p)
#####################################################################
...
# defining losses
d_loss(x, y) = binary_crossentropy(D(measure(x)), y)
g_loss(x, y) = binary_crossentropy(D(measure(G(x).data)), y)

# defining optimisers for discriminator and generator separately
d_opt = SGD(params(D), 1e-5)
g_opt = SGD(params(G), 1e-4)

d_cb = () -> println("Dicriminator loss: ", d_loss(X[:, shuffle(1:batch_size)], ones(batch_size)))
g_cb = () -> println("Generator loss: ", g_loss(randn(noise_dim, batch_size), zeros(batch_size)))
